...
class FraudReasoningV4:
    ...
    def categorize_type(self, desc: str) -> str:
        ...
        if any(kw in desc for kw in ['aircraft', 'aviation', 'aerospace']):
            ...
            return "MAINTENANCE"
        elif any(kw in desc for kw in ['research', 'development', 'r&d']):
            return "R&D"
        else:
            return "STANDARD"
    ...
    def calculate_smart_baseline(self, target: Dict) -> Dict:
        ...
        sample_penalty = 10 if len(similar) < 5 else (5 if len(similar) < 10 else 0)
        ...
        return {
            ...
            'confidence_penalty': sample_penalty
        }
    ...
    def analyze_contract(self, contract: Dict) -> FraudAssessment:
        ...
        reasoning.append(f"Sample: n={baseline['sample_size']} | Markup: {baseline['markup']:.0f}% | Z: {baseline['z_score']:.2f}")
        ...
        elif baseline['markup'] > 200 and baseline['z_score'] > 1.8:
            confidence_factors.append(85)
            legal_violations.append("False Claims Act - Price Inflation")
            reasoning.append("🚩 SUSPICIOUS: >200% markup AND >1.8σ")
        elif baseline['markup'] > 150 and baseline['z_score'] > 1.5:
            confidence_factors.append(75)
            legal_violations.append("False Claims Act - Price Inflation Indicator")
            reasoning.append("⚠️  HIGH: >150% markup AND >1.5σ")
        elif baseline['markup'] > 100:
            confidence_factors.append(65)
            reasoning.append("⚠️  ELEVATED: >100% markup")
        
        # Standard services should justify high markup
        if contract_type in ["STANDARD", "IT", "MAINTENANCE"] and baseline['markup'] > 150:
            confidence_factors.append(75)
            reasoning.append("→ Standard services >150% markup needs justification")
        ...
        if concentration > 20:
            reasoning.append(f"Concentration: {vendor_count} contracts ({concentration:.0f}%)")
        ...
        # ADJUSTED TIERS
        if confidence >= 85 and len(legal_violations) >= 2:
            tier = "🔴 RED"
        elif confidence >= 80 and len(legal_violations) >= 1:
            tier = "🔴 RED"
        elif confidence >= 65:  # LOWERED from 70
            tier = "🟡 YELLOW"
        else:
            tier = "🟢 GREEN"
    ...
    def generate_report(self):
        print("="*70)
        print("SUNLIGHT V4 - 477 CONTRACTS ANALYZED")
        print("="*70)
        ...
        if yellow:
            print("\n🟡 YELLOW TIER - WARRANTS INVESTIGATION")
            print("="*70)
        ...
        print(f"\n✅ {len(red)} high-confidence + {len(yellow)} medium-confidence cases")
        print("="*70)

if __name__ == "__main__":
    engine = FraudReasoningV4()
